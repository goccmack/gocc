...
\section{How to create and use a parser with gocc}
	...
	\begin{itemize}
		\item The user creates a target grammar conforming to the gocc BNF standard (see section~\ref{sec:gocc bnf}).
...
\section{First example}\label{sec:first example}
	...
	\subsection{The example grammar}
		...
		\begin{enumerate}
			...
			A lexical pattern may also be enclosed in square brackets, e.g.: \verb|[ '0'-'9' ]|, to indicate zero or one instances of it (in this case a decimal digit) may be present. See section~\ref{sec:gocc bnf} for details.
...
\section{gocc target BNF}\label{sec:gocc bnf}
	A gocc target grammar is written in UTF-8.  
